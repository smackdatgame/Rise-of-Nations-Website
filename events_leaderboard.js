...
const SHEET_RANGE = "A2:J"; // Columns A-J, where J (index 9) is Events retirement
...
fetch(FULL_SHEET_URL)
  ...
  .then(async rep => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Column D (index 3) = Events Points
      // Column J (index 9) = Events Retired
      const player = row[0] && row[0].v ? row[0].v : null;
      const region = row[1] && row[1].v ? row[1].v : 'NA';
      const points = row[3] && row[3].v !== null && row[3].v !== undefined ? row[3].v : 0;
      const isRetired = row[9] && (row[9].v === true || row[9].v === 'TRUE' || row[9].v === 1) ? true : false;
