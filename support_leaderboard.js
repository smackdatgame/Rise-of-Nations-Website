// support_leaderboard.js

const SHEET_ID = "1IU-KLaDjhjsyvM9NtPFSXt0HSD1rJJZnT8bEJ6klIVs";
const SHEET_TITLE = "Overall_Rank";
const SHEET_RANGE = "A2:M"; // Columns A-M, where M (index 12) is Support retirement
...
fetch(FULL_SHEET_URL)
  ...
  .then(async rep => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Column G (index 6) = Support Points
      // Column M (index 12) = Support Retired
      const player = row[0] && row[0].v ? row[0].v : null;
      const region = row[1] && row[1].v ? row[1].v : 'NA';
      const points = row[6] && row[6].v !== null && row[6].v !== undefined ? row[6].v : 0;
      const isRetired = row[12] && (row[12].v === true || row[12].v === 'TRUE' || row[12].v === 1) ? true : false;
    ...
    if (container) {
      container.innerHTML = `
        <div class="bg-red-900/50 rounded-xl px-6 py-4 text-center">
          <p class="text-red-300 font-medium">Error loading Support leaderboard data</p>
